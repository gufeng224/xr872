...
static short over_pcm[320];
static int xr872_pcm_read(int fd, void *data, unsigned int count)
{ 
   short* data_tmp = (short*)data;
   if (fd == RAW_ASR)
   {
    pcm_asound_count = snd_pcm_read(&pcm_raw_asr_config, AUDIO_CARD_ID, data_buff,2560);
   for(int i =0;i<320;i++)
   {
    data_tmp[i] = ((short *)data_buff)[4*i+2];
   }
   }
    record_output_put_data(data_tmp,640);
   //printf("pcm_asond_count ++++%d\n",pcm_asound_count);
   //`data = over_pcm;
